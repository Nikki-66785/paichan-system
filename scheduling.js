 ...
 *         APS 占用8天、由需求方按月提交后在规则月份当月内任排；规则月份无对应
 *         批次时返回 apsReminders 提醒清单（v2.17.0 起不再自动生成）；
...
(function (root) {
  ...
  Scheduler.prototype.schedule = function (reqs, existing, startSeq) {
    ...
    sorted.forEach(function (req) {
      ...
      // v2.17.0：APS（培养基模拟灌装）由需求方按月提交，不再自动生成
      if (baseType === 'APS') {
        var apsLine = req.apsLine || 'C7CM DP';
        var ym = String(req.apsMonth || '').split('-');
        var ay = parseInt(ym[0], 10), am = parseInt(ym[1], 10);
        if (!ay || !am) { ctx.warn('APS 需求缺少计划月份：' + (req.henliusProject || req.atProject || req.id), 'APS', req); return; }
        // 同月同产线已有 APS（含锁定/历史）不重复生成
        var apsDup = ctx.all.some(function (b) {
          if (b.type !== 'APS' || b.line !== apsLine || !b.start) return false;
          var d = p(b.start);
          return d.getFullYear() === ay && d.getMonth() + 1 === am;
        });
        if (apsDup) return;
        var aFirst = new Date(ay, am - 1, 1), aLast = new Date(ay, am, 0);
        var ad = new Date(aFirst), aPlaced = false;
        while (ad <= aLast) {
          var ae = add(ad, R.aps.cycle - 1);
          if (ae <= aLast && !ctx.conflicts(apsLine, ad, ae)) {
            var apsDosage = req.dosage || '水针'; // v2.18.0：APS 提交时选择剂型
            var apsBatch = {
              id: 'NB' + (ctx.seq++), reqId: req.id, seq: 1, type: 'APS', line: apsLine, dosage: apsDosage,
              stage: req.stage || '', conditionalRelease: !!req.conditionalRelease,
              start: f(ad), end: f(ae), project: req.atProject || '培养基模拟灌装', henlius: req.henliusProject || '', gmp: false,
              priority: req.priority || '中', locked: false, status: '自动', autoGenerated: false,
              note: ay + '年' + am + '月 APS ' + apsDosage + (req.conditionalRelease ? '（条件放行）' : '') +
                '（需求提交：' + (req.requester || '-') + (req.stage ? '，阶段：' + req.stage : '') + '）'
            };
        ...
        if (!aPlaced) ctx.warn(apsLine + ' ' + ay + '年' + am + '月 APS 无法插入（产线排满），请人工调整', 'APS', req);
        return;
      }
    ...
    // ---- APS 提醒（v2.17.0：不再自动排产）----
    // 规则月份内（R.aps.months）产线当年有生产、且尚无对应 APS 批次 → 返回提醒清单，
    // 由 UI 在需求录入页提示「需排产」，需求方按月在表单提交 APS 需求后由上方循环生成批次。
    ...
    Object.keys(R.aps.months).forEach(function (line) {
      ...
      Object.keys(yearSet).forEach(function (yr) {
        ...
        months.forEach(function (m) {
          if (y === curYear && m < curMon) return; // 已过去的月份不再提醒
          var has = ctx.all.some(function (b) {
            if (b.type !== 'APS' || b.line !== line || !b.start) return false;
            var d = p(b.start);
            return d.getFullYear() === y && d.getMonth() + 1 === m;
          });
          if (!has) apsReminders.push({ line: line, year: y, month: m });
        });
      });
    });

    return { batches: out, warnings: ctx.warnings, apsReminders: apsReminders };
  };
