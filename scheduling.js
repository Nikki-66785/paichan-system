 ...
 *   - DP：周期五段=准备+灌装+冻干+清场+目检包装（默认水针3+1+0+1+10=15天、
 *         冻干3+1+3~4+1+10=18~19天，其中目检包装默认10天；放大测试批次及
 *         批次类型=放大测试的需求不含目检包装（按0天计）；需二次稀配时准备+1天；
 *         各段可在人工干预时调整）；灌装日期自动=开始日期+准备天数；
...
(function (root) {
  ...
  var DEFAULTS = {
    ...
    dp: { clearGapC7: 2, clearGapSJ2: 4, diluteExtraDays: 1,
          ...
          segs: {
            ...
            'SJ2CM DP':   { 水针: [3,1,0,1,10], 冻干: [3,1,4,1,10] }
  ...
  var LINES = {
    ...
    'SJ2 ADC DP': { type: 'dp',    cycle: { 水针: 15, 冻干: 19 }, name: 'SJ2 ADC DP' },
    'C7CM DP':    { type: 'dp',    cycle: { 水针: 15, 冻干: 18 }, name: 'C7CM DP' },
    'SJ2CM DP':   { type: 'dp',    cycle: { 水针: 15, 冻干: 19 }, name: 'SJ2CM DP' }
  };
  ...
  // DP 周期五段 = 准备 + 灌装 + 冻干 + 清场 + 目检包装（排产逻辑.xlsx DP 组成）
  // 默认值按产线+剂型取自 R.dp.segs；需二次稀配时准备 +diluteExtraDays 天；
  // 放大测试批次（scaleTest）或批次类型=放大测试的需求不含目检包装（目检按 0 天计）；各段可在人工干预时调整
  function dpSegs(line, dosage, req, R, scaleTest) {
    var d = (R && R.dp) || {};
    var seg = (d.segs && d.segs[line] && d.segs[line][dosage]) || [3, 1, 0, 1, 0];
    var s = { prep: seg[0] || 0, fill: seg[1] || 0, freeze: seg[2] || 0, clear: seg[3] || 0, inspect: seg[4] || 0 };
    if (req && req.secondaryDilution) s.prep += (d.diluteExtraDays != null ? d.diluteExtraDays : 1);
    if (scaleTest || (req && req.batchKind === '放大测试')) s.inspect = 0; // 放大测试无需包装目检时间
    s.total = s.prep + s.fill + s.freeze + s.clear + s.inspect;
    return s;
  }
  ...
  function Scheduler(rules) {
    ...
    Ctx.prototype.conflicts = function (line, start, end) {
      return self.conflicts(line, start, end, this);
    };
  ...
  Scheduler.prototype.conflicts = function (line, start, end, ctx) {
  ...
  Scheduler.prototype.schedule = function (reqs, existing, startSeq) {
    ...
    sorted.forEach(function (req) {
      var baseType = req.type; // 'CB' | 'Mab' | 'ADC DS' | 'DP'
      ...
      for (var n = 1; n <= cnt; n++) {
          ...
          if (!testGen[req.id] && !lockedTest) {
            ...
            if (tf) {
              var tBatch = {
                ...
                start: f(tf.start), end: f(tf.end), project: req.atProject, henlius: req.henliusProject,
                gmp: !!req.gmp, priority: req.priority, locked: false, status: '自动',
                autoGenerated: true, note: '冻干放大测试（自动生成）',
                dpSegs: tSeg, // 周期五段（人工干预可调）
        ...
        var batch = {
          ...
          henlius: req.henliusProject, gmp: !!req.gmp, priority: req.priority,
